...
def mean_pv_from_tv(
    ...
    Args:
        ...
        theta(1,2)_deg: Spherical coordinates, where theta1 is the
            azimuth angle/longitude/phi/RA and theta2 is the
            inclination angle/90°-polar angle/theta=90°-theta2/Latitude/DEC
...
def angle_jit(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """
    Compute the angular distance between two points on a sphere using the
    spherical law of cosines.
    
    Note: All numbers must be in radians, returns radians.
    Args:
    Returns:
    """
    return np.arccos(
...
@nb.jit(nopython=True)
def vecdiff_jit(d1: float, d2: float, angle_rad: float) -> float:
    """
    Compute the difference vector between d2 and d1 using the
    law of cosine, where angle_rad is the angle between them.
    """
    return np.sqrt(d2 ** 2.0 + d1 ** 2.0 - 2.0 * d2 * d1 * np.cos(angle_rad))
