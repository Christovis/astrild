...
class TestGalaxyShapeNoise:
    def test__galaxy_shape_noise_creation(self, skymap):
    ...
    def test__galaxy_shape_noise_addition(self, skymap):
...
class TestSmoothing:
    def test__extrema_values(self, skymap):
    ...
    def test__extrema_smaller_than_original(self, skymap):
        assert np.max(skymap.data["orig_smooth"]) < np.max(skymap.data["orig"])
